...
Проект теперь поставляется с автономным десктоп‑приложением и не требует запуска web‑сервера на `localhost`.

## Архитектура
![Архитектура](docs/architecture.svg)

- **core (Rust):** базовая библиотека с опциональными модулями.
- **desktop (Rust, Iced):** автономное десктоп‑приложение, использующее `core`.
- **frontend (Vite, JavaScript):** вспомогательный web‑интерфейс и ресурсы.
...
- `frontend/` — сборка веб‑ресурсов, используемых десктопом.
- `plugins/` — плагины блоков.
- `legacy-backend/` — прежний backend, не входит в сборку по умолчанию.
...
Перед первой сборкой на Windows выполните `npm run check-env`, чтобы проверить наличие системных зависимостей.
...
Для сборки автономного десктопа выполните:

```bash
cargo build --release -p desktop
```

Библиотека `core` поддерживает выборочные сборки. Например, чтобы включить только Git и наблюдение за файлами:
...
### `gdk-3.0` not found
При сборке Tauri может появиться сообщение:

```
Package gdk-3.0 was not found in the pkg-config search path
```

Убедитесь, что установлены GTK/GLib и `pkg-config`. Проверить корректность установки можно командой:

```
pkg-config --modversion gtk+-3.0
```

Если номер версии не выводится, установите соответствующие пакеты (например, `libgtk-3-dev` и `pkg-config` в Linux или `mingw-w64-x86_64-gtk3` в MSYS2).

### Ошибки кэша
Старые артефакты сборки могут приводить к непредсказуемым ошибкам. Сначала попробуйте очистить проект:

```
npm run clean
```

Если проблема сохраняется, удалите кэш Tauri вручную и пересоберите:

```
rm -rf frontend/src-tauri/target
```

После очистки запустите сборку заново.
...
- `core` — библиотека ядра на Rust.
- `desktop` — автономное десктоп‑приложение на Iced.
- `frontend` — web‑ресурсы и клиентская логика.
- `plugins` — встроенные плагины и механизм их загрузки.
- `examples` — примеры плагинов и использования.
- `legacy-backend` — устаревший backend.
- `logs` — временные файлы и журналы (файлы вида `backend.log.YYYY-MM-DD`).
