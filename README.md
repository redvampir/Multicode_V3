...
- [Модули](#модули)
- [Синхронизация и диагностика](docs/sync.md#диагностика-несоответствий)
...
Перед началом работы выполните настройку окружения:

```bash
npm install                 # устанавливает зависимости Node.js, включая Prettier
pip install pre-commit      # подключает проверку форматирования и хуки перед коммитом
pre-commit install          # активирует хуки в локальном репозитории
```

Для обновления зависимостей используйте `npm ci` в CI и `npm install` локально.
