...
Скрипт `npm run setup` читает поле `workspaces` из корневого `package.json` и устанавливает модули каждого пакета отдельно.

## Поддержка установочных скриптов

При добавлении новых зависимостей в любой workspace обязательно обновляйте `scripts/setup.js` и связанные скрипты установки (`scripts/dev.js`, `scripts/test.js` и др.). Это гарантирует, что `npm run setup` корректно установит все пакеты.

### Проверка перед коммитом

1. Запустите `npm run setup` в корне проекта и убедитесь, что в выводе нет сообщений о пропущенных зависимостях.
2. Выполните `npm test` или другие проверки сборки, чтобы подтвердить, что проект работает с добавленными пакетами.
