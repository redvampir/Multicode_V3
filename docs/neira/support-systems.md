...
Узлы, которым требуется GPU (например, обучающие `ActionNode` или вычислительные `AnalysisNode`), объявляют необходимость через метаданные:

```json
{ "resources": { "gpu": 1 } }
```

Бюджеты намеренно устанавливаются ниже доступных системных ресурсов, чтобы не монополизировать систему и сохранять отзывчивость сервисов. Значения берутся из конфигурации планировщика и при необходимости могут масштабироваться.
