...
- [10. Итоговая документация](analysis-architecture.md)
...
- Задачи проходят состояния `queued → running → completed/failed`.
...
```
- При превышении `timeout_ms` задача отменяется. Системная функция отмены:
...
Полный обзор архитектуры анализа вынесен в отдельный документ:
