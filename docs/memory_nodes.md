...
Узлы памяти координируют работу с [узлами анализа](neira/analysis-nodes.md), поддерживая согласованность данных и контекста.
...
При длительных вычислениях взаимодействие продолжается асинхронно с помощью [TaskScheduler](neira/support-systems.md#taskscheduler). Планировщик распределяет задачи, а `MemoryNode` по мере готовности результатов обновляет контекст и метаданные.
