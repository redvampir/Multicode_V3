...
Узлы памяти координируют работу с узлами анализа; подробности см. в документе [«Узлы анализа»](neira/analysis-nodes.md).
...
При длительных вычислениях взаимодействие продолжается асинхронно с помощью планировщика задач; подробности в разделе [TaskScheduler](neira/support-systems.md#taskscheduler) файла `support-systems.md`. Планировщик распределяет задачи, а `MemoryNode` по мере готовности результатов обновляет контекст и метаданные.
