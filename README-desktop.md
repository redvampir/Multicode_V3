...
- **Conflict resolution / Разрешение конфликтов** — определяет, какая версия метаданных
  используется при расхождениях между текстом и блок‑схемой.
  Режим `PreferText` (по умолчанию) принимает текстовую версию,
  `PreferVisual` — визуальную.
- **Сохранять формат мета‑комментариев** — при обновлении кода
  оставляет исходное оформление комментариев `@VISUAL_META`.
