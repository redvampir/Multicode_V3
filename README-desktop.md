...
3. Запустите приложение:
   ...
   Приложение также принимает собственные аргументы командной строки:

   - `--project <путь>` — открыть указанный каталог проекта;
   - `--file <путь>` — открыть проект, содержащий этот файл.

   При запуске через `cargo run` аргументы приложения отделяются двойным дефисом:

   ```sh
   cargo run -p desktop -- --project path/to/project
   cargo run -p desktop -- --file path/to/file.rs
   ```
